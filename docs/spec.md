...
`hnm` is a CLI that installs a complete agent documentation harness into a target project directory. The harness is the fengwu-bench-style layout: three-layer docs (`docs/prd/`, `docs/adr/`, `docs/spec.md`), root `AGENTS.md` workflow rules, the `feature-dev` 质问 skill, Claude/Codex skill wiring, and a pre-commit documentation review gate.
